...
}$el.click(function(event){neo.Plugin.lastEvent=event;
moduleClass.select($el);
});
$el.dblclick(function(){var $ppScope=$("#properties-module-shell").scope();
...
if(neo.Page.isDirty()){neo.savePage();
}break;
...
neo.CheckoutPlugin=neo.Plugin.extend({id:"Checkout",template:'<span id="ecomm-items">0</span> items ($<span id="ecomm-price">0.00</span>) | <a href="#">Checkout</a>',construct:function($el){$el.find(".neo-asset-inner").html(neo.CheckoutPlugin.template);
}});
neo.ProductPlugin=neo.Plugin.extend({id:"Product",name:"ProductPlugin",construct:function($el){this.addCheckout();
this.checkIfNewProduct($el);
},checkIfNewProduct:function($el){if($el.hasClass("new-product")&&builderSettings.isBuilder){}else{}},addCheckout:function(){if($(".checkout-plugin").length==0){console.log("Add to cart button detected without checkout cart, adding checkout cart");
...
neo.PaymentPlugin=neo.Plugin.extend({id:"Payment",name:"PaymentPlugin"});
neo.DonationPlugin=neo.Plugin.extend({id:"Donation",name:"DonationPlugin"});
